...
// ─── Account definitions ──────────────────────────────────────────────────────
// Add / rename accounts here without touching the database.
const ACCOUNTS = [
  // Spaarrekeningen
...
export default function SavingsDashboard({ month }) {
  ...
  const [entries, setEntries]       = useState({}) // key = accountName, val = { balance, inleg }
  const [prevEntries, setPrevEntries] = useState({})
  const [editing, setEditing]       = useState(null) // accountName being edited
  ...
  SECTIONS.forEach(({ key }) => {
    sectionTotals[key] = ACCOUNTS
      .filter((a) => a.type === key)
      .reduce((sum, a) => sum + (entries[a.name]?.balance || 0), 0)
  })
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          {isAdmin && (
            <p className="text-xs text-slate-400 mt-0.5">Klik op een rekening om het saldo bij te werken</p>
          )}
        </div>
                    ...
                    <div key={account.name}>
                      {isEditing ? (
                        ...
                        <div className="flex flex-wrap items-center gap-2 py-1.5">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') save(account)
                                if (e.key === 'Escape') setEditing(null)
                              }}
                              className="w-24 border border-brand-400 rounded px-2 py-0.5 text-sm text-right focus:outline-none"
                              autoFocus
                              min={0}
                              step={100}
                            />
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <input
                              type="number"
                              value={editInleg}
                              onChange={(e) => setEditInleg(e.target.value)}
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') save(account)
                                if (e.key === 'Escape') setEditing(null)
                              }}
                              className="w-20 border border-slate-300 rounded px-2 py-0.5 text-sm text-right focus:outline-none"
                              min={0}
                              step={50}
                            />
                          </div>
                        ...
                        <button
                          ...
                          className={`w-full flex items-center gap-2 py-1.5 text-left rounded px-1 ${
                        ...
                        >
                          ...
                          <span className="text-sm font-semibold text-slate-800 w-20 text-right">
                            {e ? `€${fmt(bal)}` : (
                              <span className="text-slate-300 font-normal text-xs">— invoeren</span>
                            )}
                          </span>
