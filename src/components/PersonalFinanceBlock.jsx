...
/**
 * Private block: shows personal income, private expenses (with abonnementen subtotaal)
 * and net surplus. Only queries the logged-in user's own domain — never exposes other user's data.
 */
  ...
  async function fetchData() {
    ...
    txs.forEach((tx) => {
      ...
      if (tx.amount > 0) {
        ...
        priveUitgaven += amt
        if (tx.categories?.name === 'Abonnementen') {
          abonnementen += amt
        }
      }
    })

    setData({ inkomen, priveUitgaven, abonnementen, overschot: inkomen - priveUitgaven })
    setLoading(false)
  }
        ...
        <>
            ...
            <div className="bg-slate-50 rounded-lg p-4">
              ...
              {data.abonnementen > 0 && (
                <p className="text-xs text-slate-400 mt-1">
                  waarvan abo’s: €{data.abonnementen.toLocaleString('nl-NL', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                </p>
              )}
            </div>
          ...
          {data.overschot > 0 && (
            <p className="text-xs text-slate-400 mt-3">
              €{data.overschot.toLocaleString('nl-NL', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} beschikbaar voor sparen of overboeking naar gezamenlijke rekening.
            </p>
          )}
        </>
      )}
